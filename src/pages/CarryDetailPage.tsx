...
export default function CarryDetailPage() {
  ...
  const isOwner = carry
    ? isCurrentUserPostOwner(carry) || searchParams.get("owner") === "1" || (!isFirebaseConfigured && !isMatched)
    : false;
      ...
      <div className="relative rounded-[28px] border border-white/10 bg-[#1f2232]/90 p-5 shadow-2xl">
        ...
        {loading ? (
          ...
          <>
            ...
            <h1 className="mt-3 pr-28 text-3xl font-black leading-tight text-white sm:text-4xl">{carry.travelRoute ? routeLabel(carry.travelRoute, language) : t("Route pending", "路线待定")}</h1>
